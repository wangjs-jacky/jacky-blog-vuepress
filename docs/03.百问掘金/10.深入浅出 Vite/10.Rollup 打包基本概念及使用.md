...
在开发阶段，我们主要使用 `ESbuild` 对第三方依赖进行预构建，但是 `ESbuild` 存在种种缺陷，如不支持降级到 `es5` 代码，不支持新式语法，不具备 `ts` 类型系统机制，完全不提供处理 `chunk` 产物的接口等。因此在生产阶段，则使用功能更丰富且稳定的 `rollup` 打包工具。当然 `esbuild` 同样也可以以 `minify` 器的身份 "混入其中"。

除此以外，在 `Vite` 架构体系中，也对 `rollup` 的插件机制进行完全兼容，编写 `vite` 插件某种程度上等同于编写 `rollup` 插件，两者主要区别在于 `vite` 实现的一些特有 `hooks` 钩子函数，扩展了 `rollup` 的能力。
...
>
> 需将 `rollup.config.js` 中的 `input` 数组改为单入口模式。
...
- `tree-shaking` 的实现机制：
  ...
  `esm` 模块在当初设计时就支持静态分析，即依赖关系在运行前编译时就能确定下来。通过 `AST` 语法树对没有要的节点打标记，统一进行删除操作。
...
- 命令行：`rollup -c`  其中 `-c` 用于读取 `rollup.config.js` 配置文件。
...
- 代码编程：
  ...
  try {
    ...
    // 此时可以通过 bundle.generate 或者 write （2选1） 进行构建  
...
多入口配置：无论命令行和代码调用均支持接受一个 `input` 数组。

对于 `rollup -c` 这种命令行形式多产物配置还是很容易的，直接指定 `output` 数组格式即可。
...
对于 `rollup.rollup` 这种代码调用格式，`bundle.write({})` 无法传入数组，必须一个对象格式，多产物只能手动循环实现。
...
`umd` 格式的打包，需要单独区分，因为此类格式要求单入口+单出口。需要将 `output` 中的 `dir` 修改为 `file` 格式才可以。

> 这种配置方案，查了半天都没有配置出来，最终还是问 `chatgpt` 才解决的。
>
> 可使用仓库代码中  `npm run build:umd` 指令测试。
...
对于打包工具所具备能力都很类似，如 `fomat`、`sourcemap`，当用到时再去官网搜索就好了。
...
### 问题7：`rollup` 怎么接入 `plugin` 插件？

配置 `plugin` 插件时首先要区分插件的运行时机是在 **开发阶段** 还是**生产阶段**？

如果分不清的话，可直接将所有插件配置到最外层的  `plugins` ，如果明显为生产阶段的插件，可以配置到 `output` 中。
...
### 问题8：如何打包 `loadsh` 这类 `cjs` 格式包，在开发实践中都使用过哪些常用的Rollup库？
